...
```
├── install/                              # AI 引導安裝腳本（貼給 AI 讀）
│   ├── zh-TW.md  zh-CN.md  en.md
│
├── claude-code/                          # Claude Code 專用
│   ├── hooks/
│   │   └── block-chained-bash.py         # 擋 && || ; 串接的 PreToolUse hook
│   ├── starter-allowlist.json            # 常用安全指令白名單（併進 settings.json）
│   ├── zh-TW/
│   │   ├── CLAUDE.md                     # 行為規則（含 Shell 不串接規則）
│   │   └── output-styles/
...
├── codex/                                # Codex CLI 專用
│   ├── zh-TW/
│   │   ├── AGENTS.md                     # 行為規則 + 完整 Output Style（含 Shell 軟規則）
...
| **回覆格式** | `output-styles/concise-structured.md` → `~/.claude/output-styles/` | Project Level 在 `AGENTS.md`；User Level 完整寫入 `config.toml` 的 `instructions` |
| **強化設定** | `~/.claude/settings.json` 設 `outputStyle`；`/config` 可用時也能切換 | `~/.codex/config.toml` 加 `personality` + 完整 `instructions` |
...
不想自己複製檔案？把下面這句貼給 Claude Code 或 Codex，AI 會先問你要裝哪個工具、哪個層級，再照步驟裝好（含行為規則、回覆格式、Shell 不串接 hook、常用指令白名單），最後帶你逐項驗證：

```
Read jr_ai_agent_configs/install/zh-TW.md and guide me through installing these configs. 先問我要裝哪個工具和哪個層級，再照步驟裝，最後帶我驗證。
```

> 換語言就改路徑：`install/zh-CN.md`（簡中）、`install/en.md`（英文）。
...
再把 `"outputStyle": "Concise Structured"` 合併進 `~/.claude/settings.json`，然後 `/clear` 或開新 session。不要只依賴 `/config` 選單：部分版本不會列出自訂 style，`/output-style` 也可能不是有效指令。完整的備份與 JSON 合併步驟請用上方 AI 引導安裝。
...
> **想全域套用行為規則？** 目前 Codex 沒有全域 `AGENTS.md` 機制；`config.toml.example` 已把回覆格式、預期追問與 Shell 不串接軟規則完整寫進 `instructions`。專案特有規則仍應放在各專案的 `AGENTS.md`。
...
| 驗證項目 | 看什麼 | 沒裝成功的樣子 |
|---|---|---|
| **結論先行** | 第一行在終端畫面中是粗體推薦結論 | 「好問題！讓我幫你分析一下...」 |
...
Output Style 選單、Shell 測試或權限提示和預期不同時，請看 [安裝驗證與常見問題](docs/verification-troubleshooting.md)。
